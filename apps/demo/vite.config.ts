...
function docsPlugin() {
	return {
		...
		configureServer(server: { middlewares: { use: (fn: (req: any, res: any, next: () => void) => void) => void }; config: { publicDir: string } }) {
			server.middlewares.use((req: { url?: string; method?: string }, res: { setHeader: (k: string, v: string) => void; end: (body?: string) => void; statusCode: number }, next: () => void) => {
